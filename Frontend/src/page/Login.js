...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (email && password) {
      ...
      const fetchData = await fetch(`${process.env.REACT_APP_API_URL}/login`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-3 md:p-4">
      <div className="w-full max-w-md bg-white m-auto flex flex-col p-4">
        <div className="w-20 m-auto overflow-hidden rounded-full drop-shadow-md">
          <img src={signi} className="w-full" alt="login-animation" />
        </div>
